...
function Search() {
  ...
  let search = 3; // 검색할 데이터
  let flag = false; // 플래그 변수: 찾으면 true 찾지못하면 false
  let index = -1; // 인덱스 변수: 찾은 위치

  //[2] Process(처리): 이진 검색(Binary Search): Full Scan -> Index Scan
  ...
  while (low <= high) { // low와 high가 만날 때까지 반복 실행
    ...
    let mid = parseInt((low + high) / 2); // 중간 인덱스(mid) 구하기
